...
def _compare(args, tile, monkeypatch):
    ...
    for setting in (0, tile):
        monkeypatch.setattr(attn, "_PREFILL_TILE_Q", setting)
        attn.minimax_m3_sparse_attn(**args)
        outputs.append(args["output"].clone().float())
    tol = 1e-4 if args["q"].dtype == torch.float32 else 2e-2
    errors = [(x - reference).abs().max().item() for x in outputs]
    errors.append((outputs[0] - outputs[1]).abs().max().item())
    print(
        f"dtype={args['q'].dtype} tile={tile} "
        f"max_abs(original/ref,tiled/ref,pair)={errors}"
    )
    for out in outputs:
        torch.testing.assert_close(out, reference, rtol=0, atol=tol)
    torch.testing.assert_close(outputs[0], outputs[1], rtol=0, atol=tol)
